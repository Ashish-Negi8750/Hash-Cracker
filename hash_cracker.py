...
def validate_hash(hash_value, algo):
    ...
    if algo is None:
        return False
    if len(hash_value) != expected_lengths.get(algo, 0):
        return False
...
def mutate(word, rules):
    ...
    for rule in rules:
        ...
        # you can add more built-in rules here
...
# --- Brute Mode (single-threaded safe default) ---
def run_brute(target_hash, algo, charset, maxlen, verbose):
    """
    Returns (found, attempts).
    Single-threaded brute-force to remain portable and predictable.
    If you want multiprocessing for speed, I can add a batched Pool-based version.
    """
    if not charset:
        charset = "abcdefghijklmnopqrstuvwxyz0123456789"
    if not maxlen or maxlen < 1:
        maxlen = 4  # safe default
    attempts = 0
    found = None
...
def main():
    ...
    if args.hash:
        ...
        if not os.path.exists(args.hashfile):
            console.print(f"[red]Error: Hash file not found: {args.hashfile}[/red]")
            return
        with open(args.hashfile, "r", encoding="utf-8", errors="ignore") as f:
            hashes = [line.strip() for line in f if line.strip()]
    else:
        console.print("[red]Error: You must provide --hash or --hashfile[/red]")
        return

    for hash_value in hashes:
        algo = args.algo or detect_algo(hash_value)
        if algo is None:
            console.print("[red]Could not detect algorithm from the hash length. Use --algo to specify.[/red]")
            if not args.force:
                continue
        ...
        if args.mode == "dictionary":
            ...
            if not args.wordlist or not args.wordlist2:
                console.print("[red]Error: --wordlist and --wordlist2 are required for combo mode[/red]")
                continue
    ...
    console.print("[green]Run complete. Check logs/cracked.json for appended results.[/green]")
